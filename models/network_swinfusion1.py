...
class SwinFusion(nn.Module):
    ...
    def forward(self, A, B):
        ...
        # 最后一个mse保留的情况下：
        # 把F1和Unet的输出做蒸馏。（相当于多加一个损失函数，往前走几层？）
        # 输出的F1， 通过一个网络（两层或者三层），以不同权重加到不同的Unet的decoder层上。
        # 输出train_log 每一种损失函数都要输出
        # 做test的时候，算教师模型和学生模型的损失函数（用loss_vif），并记录下来
        x = x / self.img_range + self.mean
        return x[:, :, :H*self.upscale, :W*self.upscale], feature_map
...
if __name__ == '__main__':
    ...
    model = SwinFusion(upscale=2, img_size=(height, width),
                   ...
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect')  # upsampler is 'pixelshuffledirect'
    print(model)
    # print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 1, 136, 136))
    y = torch.randn((1, 1, 136, 136))
    x, temp = model(x, y)
    print(temp.shape)
